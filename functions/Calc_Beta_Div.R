...
Calculate_Beta_div <- function(otumat_2g, div_type){
  ...
  for(i in 1: nrow (taxmat_2g)){
    ...
    if(div_type == "3"){
      Beta_diver_Bray_Curtis <- vegdist(t(as.matrix(otumat_2g)), method = "bray")
      text_tmp <- "Beta diversity on Bray–Curtis dissimilarity is calculated"
      return(list(beta_div = Beta_diver_Bray_Curtis, log_text = text_tmp))
    }
    if(div_type == "4"){
      Beta_diver_Bray_Curtis <- vegdist(t(as.matrix(otumat_2g)), method = "horn")
      text_tmp <- "Beta diversity on Horn–Morisita index is calculated"
      return(list(beta_div = Beta_diver_Bray_Curtis, log_text = text_tmp))
    }
    if(div_type == "5"){
      Beta_diver_Bray_Curtis <- vegdist(t(as.matrix(otumat_2g)), method = "jaccard")
      text_tmp <- "Beta diversity on Jaccard index is calculated"
      return(list(beta_div = Beta_diver_Bray_Curtis, log_text = text_tmp))
    }
