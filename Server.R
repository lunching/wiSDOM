...
server = function(input, output, session){
  ...
  observeEvent(input$action_RA_level, {
    val <- dataInput_RA_level()[[3]]
  ...
  data_SCA_plot <- eventReactive(input$action_m1_SAC,{
    ...
    Species_Accu_Curve(OTU_input = dataInput_RA_level()[[4]], n_rep = input$n_replications_m1_SAC, n_sample = input$n_sample_m1_SAC)
  })
  ...
  data_RAC_plot <- eventReactive(input$action_m1_RAC,{
    ...
    Rank_abundance_curve(OTU_input = dataInput_RA_level()[[4]], group_index = dataInput_RA_level()[[5]], by = "group")
  })
  ...
  data_Alpha_Div_plot <- eventReactive(input$action_m2_alpha_diversity,{
    ...
    Boxplot_alpha_diversity(OTU_input = dataInput_RA_level()[[4]], group_index = dataInput_RA_level()[[5]], index_method = input$select_alpha)
  })
  ...
  #  labels_data_type<- input$input_OTU$name
  #  if(labels_data_type == input$input_OTU$name){
  #    Test_alpha_diversity(OTU_input = dataInput_bar1()[[4]], group_index = dataInput_bar1()[[5]], index_method = input$select_alpha)
  #  }else{
  #    Test_alpha_diversity(OTU_input = dataInput_RA_level()[[4]], group_index = dataInput_RA_level()[[5]], index_method = input$select_alpha)
  #  }
  #})
  ...
  observe({
    labels_data_type <- "No data selected! please load the data with full information of taxonomy first"
    #if(input$select_RA_type != "1"|input$select_RA_type != "2"){
    if(any(c(3,4,5,6,7,8)==input$select_RA_type) ){
      labels_data_type <- "No data selected! please load the data with full information of taxonomy first"
    }
    else{
      labels_data_type<- input$input_RA_level$name
